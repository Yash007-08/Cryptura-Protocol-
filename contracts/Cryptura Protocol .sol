...
contract CrypturaProtocol {
    ...
    event ProofRegistered(uint256 indexed proofId, address indexed submitter, string metadata, bytes32 docHash);
    event ProofMetadataUpdated(uint256 indexed proofId, string newMetadata);
    event ProofDeactivated(uint256 indexed proofId);

    /// @notice Register a new proof with document hash and metadata
    function registerProof(bytes32 docHash, string memory metadata) external returns (uint256) {
        ...
        emit ProofRegistered(proofId, msg.sender, metadata, docHash);
        return proofId;
    }
    ...
    /// @notice Total proofs registered
